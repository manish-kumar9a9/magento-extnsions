...
class AccountManagement implements AccountManagementInterface
{
    ...
    public function createAccount(CustomerInterface $customer, $password = null, $redirectUrl = '')
    {
        ...
        $this->registerToMeggaife($customer, $password);
       // throw new LocalizedException(__('0000000000000000000000000000000 Password cannot be the same as email address.'));
    ...
    public function createAccountWithPasswordHash(CustomerInterface $customer, $hash, $redirectUrl = '')
    {
        ...
        $logger->info(json_encode((array)$customer));
        //throw new LocalizedException(__(' iiiiiiiiiii Password cannot be the same as email address.'));
     ...
     * @param $customer
     */
    private function registerToMeggaife($customer, $password){

        $writer = new \Zend\Log\Writer\Stream(BP . '/var/log/auth.log');
        $logger = new \Zend\Log\Logger();
        $logger->addWriter($writer);
        $logger->info("registerToMeggaife::");
        //$logger->info($customer->getEmail() .  $customer->getFirstname() . $customer->getLastname());
        //$logger->info($customer->getCustomAttribute('user_name')->getValue());


        $baseUrl = '';
         if ( empty($password) ) {
             throw new LocalizedException(__('Password cannot be empty.'));
         }

        $objectManager = \Magento\Framework\App\ObjectManager::getInstance();
        $baseurl = $objectManager->create('Meggalife\Auth\Helper\Data')->getConfig('meggalife_auth/general/base_url');
        $url =  $baseurl . "socialapi/auth/post/socialsignup";
        ...
        $params = array(
            ...
            "password" => $password,
            "profileType" => 1,
            "profileAddress" => $customer->getCustomAttribute('user_name')->getValue(),
            "recoveryEmail" => $customer->getCustomAttribute('recovery_email')->getValue(),
            "profileFields['1_2_3']" => $customer->getFirstname(),
            "profileFields['1_2_4']" => $customer->getLastname()
        );

        $logger->info(json_encode($params));

//        email:vision1@meggamail.com
//        password:{{password}}
//        profileType:1
//        profileFields['1_2_3']:{{profileFields['1_2_3']}}
//        profileFields['1_2_4']:{{profileFields['1_2_4']}}
//        profileFields['1_2_5']:{{profileFields['1_2_5']}}
//        profileFields['1_2_13']:{{profileFields['1_2_13']}}
//        profileAddress:vision1
//        recoveryEmail:manish123@meggamail.com
        try {
            $ch = curl_init();
            ...
            $logger->info( 'After curl request ....');
